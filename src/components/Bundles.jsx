...
function Bundles() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await API.get("/api/products?category=bundle");
    ...
    <div>
      <div className="w-full min-h-screen px-[2vw] bg-[#FCF7E6] pb-[15vh]">
        <ProductsHeader
          title={"Bundles."}
          description={
            "We've made exploring our range and equipment as easy as possible with our recommended bundles. Ideal for home or as a gift for any coffee expert."
          }
        />
        <div className="products grid grid-cols-3 max-sm:grid-cols-1 gap-[3vw] max-sm:gap-[2vw] 
